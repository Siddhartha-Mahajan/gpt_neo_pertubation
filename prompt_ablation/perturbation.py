...
def find_c_proj(last_block: nn.Module, embed_dim: int, target_name: str | None = None) -> tuple[nn.Linear, str]:
    ...
    if c_proj is None:
        raise RuntimeError("Could not find c_proj in last block.")
...
def run(
    ...
    c_proj, c_proj_name = find_c_proj(last_block, embed_dim=head_dim * n_heads, target_name=target_cproj_name)
    logger.info("Using c_proj='%s' with shape=%s", c_proj_name, tuple(c_proj.weight.shape))

    results: List[dict] = []
    logger.info("Prompt-ablation sweep across %d heads (noise_eps=%.4f, repeats=%d, zeroing=%s)", n_heads, noise_eps, repeats, zeroing)
    ...
    logger.info("Finished ablation sweep. Top rows:\n%s", df_results.head())
    return df_results
